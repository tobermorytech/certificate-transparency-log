	...
	context "for a precert" do
		...
		context "body" do
			...
			it "has the right leaf_input" do
				expect(Digest::MD5.digest(body['entries'][0]['leaf_input'])).
				  to eq("c\xFBN\x0E\xEA\xF7\xD1\xC7\x88X#\x83\xDDkD\xAF")
			end

			it "has the right extra_data" do
				expect(Digest::MD5.digest(body['entries'][0]['extra_data'])).
				  to eq("\xE2\x99W\x9BMia\xEC\xC0\x85\x97\x88!;\xCC\xA8")
			end
		end
	end
end
